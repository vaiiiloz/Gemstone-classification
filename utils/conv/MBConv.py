...
class Conv2dStaticSamePadding(nn.Conv2d):
    """2D Convolutions like TensoFlow, for a fixed image size"""
...
class MBConvBlock(nn.Module):
    ...
    def __init__(self, ksize, input_filters, output_filters, expand_ratio = 1, stride=1, image_size=224):
        ...
        if self._expand_ratio != 1:
            ...
            self._bn0 = nn.BatchNorm2d(num_features=oup, momentum=self._bn_mon, eps = self._bn_eps)
            
        #Depthwose convolution
        k = self._kernel_size
        s = self._stride
        Conv2d = get_same_padding_conv2d(image_size=image_size)
    ...
    def forward(self, inputs, drop_connect_rate=None):
        ...
        #Expansion adn Depthwise Convolution
        x = inputs 
        if self._expand_ratio != 1:
